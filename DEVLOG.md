...
### 块 2 HTML 报表 Polish（25 分钟）
3 项调整全部生效：
- 异常数颜色编码（= 0 绿 #1a7f37 / > 0 红 #cf222e）
- 5 个章节加英文副标题（Class Distribution / Per-Class Statistics 等）
- 饼图标签精简（移除框数 + 加脚注澄清"按检测框数统计"）

报表视觉评分：Day 14 4/5 → Day 15 4.5/5

### 块 1 ONNX 导出 + 性能基准（70 分钟）

#### 模型导出
- best.pt (6.0 MB) → best.onnx (11.7 MB, 1.96× 大小)
- 大小膨胀原因：ONNX 含完整计算图 + op 定义（PyTorch 假设 Python 代码可用）
- opset=12, simplify=True（最大兼容性）

#### 一致性验证（5 张图，100% 匹配）
- 框数完全匹配：5/5
- 最大坐标差 0.06 像素（浮点舍入）
- 证明 ONNX 推理结果与 PyTorch 完全等价

#### 性能基准（90 张图，10 张 warmup）

| 指标 | PyTorch GPU | ONNX CPU |
|---|---|---|
| 平均推理 | 19.6 ms | 39.9 ms |
| P50 | 18.9 ms | 39.7 ms |
| P95 | 25.0 ms | 42.0 ms |
| P99 | 29.8 ms | 53.8 ms |
| FPS | 51.1 | 25.1 |
...
- 25 FPS @ 200×200 已覆盖典型产线节拍 (1-10 张/秒)
- 意味着无 GPU 工业 PC 也能实时部署
- GPU/CPU 仅 2× 差距（远低于常见的 10-20×）— 因为本项目选 YOLOv8n + 200×200 小图，CPU 友好
- 教训：模型选型应考虑部署目标（不是越大越好）

### Insight #12: 一致性验证是工程必需
- 很多 ONNX 转换跳过一致性测试 → 部署后发现精度退化才返工
- 0.06 px 误差证明转换无损
- 工程原则：模型格式转换必须做"同输入同输出"验证

### 简历素材
版本 A（事实陈述）：
"完成 PyTorch→ONNX 模型部署链路：best.pt (6MB) → best.onnx (11.7MB)，
一致性验证通过（框数精确匹配，坐标误差 < 0.06px）。
性能基准：ONNX CPU 25 FPS (39.9ms/img)，满足工业产线 1-10 张/秒节拍。"

版本 B（含工程洞察）：
"实测 YOLOv8 模型完整部署链路：
- 训练 (PyTorch GPU 51 FPS) → ONNX 导出 → CPU 端推理 (25 FPS)
- 模型选型 YOLOv8n + 200×200 输入：CPU 推理友好，GPU/CPU 仅 2× 差距
